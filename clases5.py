...
class Hotel(Restaurante):                               #Subclase Hotel que hereda de la clase Restaurante
    ...
    def __init__(self, nombre, categoria, precio, piscina):      #Constructor subclase
        super().__init__(nombre, categoria, precio)              #Sintaxis para heredar los atributos de la clase Restaurante
        self.piscina = piscina

    def mostrar_informacion(self):                               #Mostrar el metodo mostrar_informacion con los atributos heredados y uno exclusivo de Hotel (piscina)
        print(f'Nombre: {self.nombre}, Categoria: {self._categoria}, Precio: {precio_hotel}, Piscina: {self.piscina}')

hotel = Hotel('Hotel PTA', '5 Estrellas', 200, 'Si')
precio_hotel = hotel.get_precio()                       #Para poder mostrar el precio PRIVATE de Hotel
